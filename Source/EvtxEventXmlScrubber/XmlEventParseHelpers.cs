...
namespace Microsoft.EvtxEventXmlScrubber
{
    ...
    /// </summary>
    public static class XmlEventParseHelpers
    {
        /// <summary>
        ...
        internal static KeyValuePair<string, object> GetXmlSystemNameValue(XElement xElement, string attributeName)
        {
            try
            {
                switch (attributeName)
                {
                    ...
                    case "TimeCreated":
                        return new KeyValuePair<string, object>(attributeName, Convert.ToDateTime(xElement.FirstAttribute.Value).ToUniversalTime());
                    case "Security":
                    ...
                    case "EventID":
                        return new KeyValuePair<string, object>("EventId", Convert.ToInt32(xElement.Value));
                    case "EventRecordID":
                        return new KeyValuePair<string, object>("EventRecordId", Convert.ToInt64(xElement.Value));
                    case "Version":
                    case "Level":
                    case "Task":
                    case "Opcode":
        ...
        public static Dictionary<string, object> ConvertSystemPropertiesToDictionary(XElement xmlElement)
        {
            ...
            // Winlog older format events dont have some properties. We need to add dummy properties to 
            // normalize the events for consumption
            foreach (var data in systemData.Descendants())
            {
                ...
                {
                    // Version Attribute should be present in the dictionary before Level property 
                    if (!systemPropertyDictionary.ContainsKey("Version"))
                    {
                        systemPropertyDictionary.Add("Version", 0);
                    }
                }

                if (attributeName == "Keywords")
                {
                    // Opcode Attribute should be present in the dictionary before Keywords property
                    if (!systemPropertyDictionary.ContainsKey("Opcode"))
                    {
                        systemPropertyDictionary.Add("Opcode", 0);
                    }
                }

                if (attributeName == "Channel")
                {
                    // Following Attributes should be present in the dictionary before Channel property
                    if (!systemPropertyDictionary.ContainsKey("Correlation"))
                    {
                        systemPropertyDictionary.Add("Correlation", null);
                    }

                    if (!systemPropertyDictionary.ContainsKey("ProcessId"))
                    {
                        systemPropertyDictionary.Add("ProcessId", 0);
                    }

                    if (!systemPropertyDictionary.ContainsKey("ThreadId"))
                    {
                        systemPropertyDictionary.Add("ThreadId", 0);
                    }
                }

                if (attributeName == "Execution")
                {
                    systemPropertyDictionary.Add("ProcessId", Convert.ToInt32(data.FirstAttribute.Value));
                    systemPropertyDictionary.Add("ThreadId", Convert.ToInt32(data.FirstAttribute.NextAttribute.Value));
                }
                else
                {
