...
namespace WinLog
{
    ...
    /// </summary>
    public class LogReaderLogRecord
    {
        ...
        public IEnumerable<LogRecord> ReadFile(string logPath, bool useJsonFilter, string jsonFilter, long logFileId, string collector,
        ...
        public IEnumerable<LogRecord> ReadLog(string logName, EventBookmark bookmark)
        {
            var log = EvtxEnumerable.ReadWindowsLog(logName, bookmark);
            return ReadAsJson(log);
        }

        private IEnumerable<LogRecord> ReadAsJson(IEnumerable<EventRecord> log)
        {
            ...
            foreach (var e in log)
            {
                ...
                yield return new LogRecord(e.Bookmark)
                {
